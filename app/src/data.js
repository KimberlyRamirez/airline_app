...
const getAirlineName =  (obj) => {
  let objId = obj.airline
  return airlines.filter(({id}) => id === objId)[0].name
}

function getAirlineByCode(obj, property) {
  let objCode = obj[property]
  return airports.filter(({code}) => code === objCode)[0].name
}

const getAirlineId = (givenName) => {
  return airlines.filter(({name}) => name === givenName)[0].id
}

const getAirportCode = (givenName) => {
  return airports.filter(({name}) => name === givenName)[0].code
}


export default {routes, airlines, airports, getAirlineName, getAirlineByCode, getAirlineId, getAirportCode};